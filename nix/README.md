...
([`lua/`](../lua/)) is the config-directory content this wraps; see
...
- [`default.nix`](wrapper/default.nix) — the wrapper module: imports every language's
  ...
  `readDir`), sets `config.settings.config_directory` (points Neovim at this repo's Lua
  on disk for live-reload dev iteration instead of copying it into the Nix store), and
  defines the `runtimePkgs` spec field used to suffix binaries onto `$PATH`.
- [`plugins.nix`](wrapper/plugins.nix) — `config.specs.general`: the always-on plugin
  list, matching [`lua/plugins/`](../lua/plugins/), independent of language.
- [`tools.nix`](wrapper/tools.nix) — `config.specs.tools`: general-purpose
  `runtimePkgs` not tied to any plugin or language (`lazygit`, `tree-sitter`).
...
`nixpkgs#vimPlugins` — useful when wiring a new plugin into
[`plugins.nix`](wrapper/plugins.nix) or a language's `default.nix` and you need the
